...
Sequence of commands when u dopwnload the project 
...
uv run python main.py --all_symbols --no-loop --Retreve_document --first-run
...
| Metadata JSON generator      | —                             | In progress            |
...
| Export    | `exporter.py`   | normalized data      | final artifacts          | In progress (planned) |
...
| `company_metadata.json`                    | In progress                                       |
...
| Per-symbol markdown output                 | In progress                                       |
...
│   └── exporter.py                  # In progress (planned)
├── storage/
...
1. Set API key (do not commit; use `.env` locally):
  ```bash
   export INDIAN_API_KEY=***   # or
   export INDIA_API_KEY=***   
  ```
